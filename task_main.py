...
if __name__ == "__main__":
    ...
    if check_taskstatus('GETTING_STOCKDATA1', ticker) == 'NotExecuted' or 'Error' :
        try :
            get_asset_value(ticker, date_begin, date_end)
            update_taskstatus('GETTING_STOCKDATA1', ticker, 'Completed')
        except Exception as e :
            update_taskstatus('GETTING_STOCKDATA1', ticker, 'Error')

    if check_taskstatus('GETTING_STOCKDATA2', ticker) ==  'NotExecuted' or 'Error' :
        try :
            preprocess_asset_value(ticker, date_begin, date_end)
            update_taskstatus('GETTING_STOCKDATA2', ticker, 'Completed')
        except Exception as e :
            update_taskstatus('GETTING_STOCKDATA2', ticker, 'Error')
        ...
        for _ in range(retry_count) :
            ...
            date_begin_test, date_end_test, date_begin_train, date_end_train, \
                accuracy, loss, mse = build_model(data_set,
                                            hyperparameter_set)
            update_taskstatus(code_task, ticker, 'Completed')
